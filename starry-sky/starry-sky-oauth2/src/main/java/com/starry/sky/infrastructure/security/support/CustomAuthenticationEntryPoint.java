...
import com.starry.sky.infrastructure.config.CustomOauthServerConfig;
import com.starry.sky.infrastructure.config.CustomWebSecurityConfig;
...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    private final AuthenticationEntryPoint loginUrlAuthenticationEntryPoint = new LoginUrlAuthenticationEntryPoint(CustomWebSecurityConfig.DEFAULT_LOGIN_URL);
...
//        }
        if (authException instanceof CustomAuthenticationException){
            ...
            log.debug("CustomAuthenticationEntryPoint throws CustomAuthenticationException :\t{}",
                    customAuthenticationException.getMessage());
            ResultData resultData = ResultData.customizeResult(customAuthenticationException.getCode(),
                    customAuthenticationException.getMessage());
            ResultData.printJson(response, objectMapper.writeValueAsString(resultData));
            return;
            ...
            log.debug("CustomAuthenticationEntryPoint throws InsufficientAuthenticationException :\t{}",
                    insufficientAuthenticationException.getMessage());
            ResultData resultData = ResultData.customizeResult(ResultCode.ACCESS_DENIED_NO_PERMISSION);
            ResultData.printJson(response, objectMapper.writeValueAsString(resultData));
            return;
        }
